...
public class OKAudioListener : MonoBehaviour {
    ...
    void LateUpdate() {
        if (processAudioThread == null || !processAudioThread.IsAlive) return;
    ...
    void ProcessAudio() {
        ...
        float prevSample = samples[frameLen - 1];
        Array.Fill(samples, 0f);
        frameLen = OKAudioManager.GetFrameLen();
